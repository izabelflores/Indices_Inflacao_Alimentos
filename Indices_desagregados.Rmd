...
Precos_subgrupos <- IPCAalim %>%
  dplyr::select(-`11.Alimentação no domicílio`,
...
precos_C_IND <- Precos_subgrupos %>% 
  dplyr::select(mes_cod,
...
precos_C_SEMI <- Precos_subgrupos %>% 
  dplyr::select(mes_cod,
                                             `1101002.Arroz`,
...
precos_NC_IND <- Precos_subgrupos %>% 
  dplyr::select(mes_cod,
...
precos_NC_SEMI <- Precos_subgrupos %>% 
  dplyr::select(mes_cod,
...
peso_C_IND <- Pesos_subgrupos %>% dplyr::select(mes_cod,
                                         `1102001.Farinha de arroz`,
                                         `1102006.Macarrão`,
                                         `1102008.Fubá de milho`,
                                         `1102009.Amido de milho`,
